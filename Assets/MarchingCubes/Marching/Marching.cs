...
namespace MarchingCubesProject
{
    public abstract class Marching //: IMarching
    {
        ...
        public virtual void Generate(IList<VoxelData> voxels, int width, int height, int depth, IList<Vector3> verts, IList<int> indices)
        {
                ...
                for (y = 0; y < height - 1; y++)
                {
                    for (z = 0; z < depth - 1; z++)
                    {
                        ...
                        for (i = 0; i < 8; i++)
                        {
                            ...
                            Cube[i] = voxels[ix + iy * height + iz * width * depth].Value;
                        }
